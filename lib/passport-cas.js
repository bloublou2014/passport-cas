...
function CasStrategy(options, verify) {
    ...
    if (!verify) {
        throw new TypeError('CasStrategy requires a verify callback');
    }
    if (!options.casURL) {
        throw new TypeError('CasStrategy requires a casURL option');
    }
...
CasStrategy.prototype.authenticate = function (req, options) {
    ...
    service = url.format({
        protocol: 'https', // req.headers['x-proxied-protocol'] || req.protocol || 'https',
    ...
    if (!ticket) {
        ...
        self.cas.validate(ticket, function (err, status, username, extended) {
            ...
            // Validation successful
                ...
                const attributes = extended.attributes;
                const profile = {
                    provider: 'CAS',
                ...
                if (extended && extended.raw) {
                    profile.raw = extended.raw;
                }

                // Map relevant extended attributes returned by CAS into the profile
                for (let key in profile) {
                    if (key === 'name') {
                        for (let subKey in profile[key]) {
                            let mappedKey = self.casPropertyMap[subKey] || subKey;
                            let value = attributes[mappedKey];
                        ...
                        let mappedKey = self.casPropertyMap.emails || 'emails';
                        ...
                        let mappedKey = self.casPropertyMap[key] || key;
                        let value = attributes[mappedKey];
                        if (Array.isArray(value)) {
                            profile[key] = value[0];
                        } else if (value) {
 ...
 * @param {String} pgtURL
 *      The URL of this PGT callback server. It must use
 and be accessible
